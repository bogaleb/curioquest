...
import { ArrowRight, Check, Heart, LockKeyhole, Play, Search, Volume2 } from "lucide-react";
...
export function GameZone({profile,busy,onStart,onFavorite,onContinue,onResume}:Props) {
  ...
  return <div className="game-zone"><section className="arcade-hero"><div>
    ...
    <div className="playground-guide"><div><strong>Pick a game. Press Play. Explore!</strong><p>You can switch games whenever you like. Completed discoveries are saved automatically.</p></div></div>
...
export function GameZoneInvitation({onOpen}:{onOpen:()=>void}) {
  return <section className="game-zone-invitation"><div className="invitation-toys" aria-hidden="true">🤖 🥞 🚂</div><div><h2>Welcome to the Game Zone</h2><p>Eight playful collections. Robots, stories, puzzles, and tiny discoveries.</p></div><button className="primary" onClick={onOpen}>Let’s play<ArrowRight size={19}/></button></section>;
}
