...
export function useCurio(initial:CastState='idle'){
 const [state,setState]=useState<CastState>(initial),timer=useRef<ReturnType<typeof setTimeout>|null>(null);
 useEffect(()=>()=>{if(timer.current)clearTimeout(timer.current);},[]);
 const play=useCallback((next:CastState)=>{if(timer.current)clearTimeout(timer.current);setState(next);if(next!=='idle')timer.current=setTimeout(()=>setState('idle'),next==='dance'?3200:2000);},[]);
