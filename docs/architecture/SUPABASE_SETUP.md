...
Apply migrations before the matching app release. Hosted learning/Storage checks passed; browser QA, real signup/recovery email delivery, the first GitHub workflow run and any legacy transfer remain release gates. No Vercel deployment has been performed for this migration. After Supabase accepts new writes, rollback requires reconciling those writes with the backup, not merely reverting code.
