...
export default function WorldsPage() {
  ...
  return (
            ...
            <div className="content-grid"><div><div className="section-heading"><h2>Your learning worlds</h2></div><div className="worlds-grid">{worlds.map(w => <button key={w.id} disabled={busy || profile.preferences.paused} className="world-card" data-world={w.id} onClick={() => start(w.id)}><div className="world-top"><span className="world-symbol"><w.icon size={35} strokeWidth={1.6}/></span><span className="world-tag">{subjectRegistry.find(subject=>subject.id===w.id)?.eyebrow}</span></div><h3>{w.name}</h3><p>{w.skill}</p><div className="world-progress"><span>Trail {profile.skills[w.id].level} of 3</span><ArrowRight size={18}/></div><div className="progress-track"><span style={{ width: `${profile.skills[w.id].seen ? Math.min(100, profile.skills[w.id].seen / 15 * 100) : 0}%` }}/></div></button>)}</div>
            ...
            <div className="future-worlds"><strong>More worlds are on the horizon</strong><p>Design a garden in Build Lab, or follow Nova through the Missing Seeds story.</p></div></div>
            <aside className="right-column"><section className="today-card"><div className="section-heading"><h3>A little progress</h3><span className="sun-icon">☀</span></div><p>Small steps add uprofile.</p><div className="daily-progress"><span className="ring"><Check size={24}/></span><div><strong>{today > 0 ? 'You explored today!' : 'A fresh start'}</strong><span>{today > 0 ? `${today} quest${today === 1 ? '' : 's'} completed` : 'Your first discovery awaits'}</span></div></div><div className="today-footer"><span><Star size={17}/>{profile.stars} stars earned</span><span><Flag size={17}/>{profile.completed} quests</span></div></section><section className="nova-card"><div className="nova-card-title"><span>🦊</span><div><strong>A note from Nova</strong><small>YOUR FRIENDLY GUIDE</small></div></div><p>“You don’t have to know the answer yet. Let’s figure it out together!”</p><button className="text-button" onClick={() => speak('You don’t have to know the answer yet. Let’s figure it out together!')}><Volume2 size={17}/> Listen to Nova</button></section><div className="gentle-note"><Leaf size={17}/><span>No rush. No races.<br />Just your kind of adventure.</span></div></aside></div></>
  );
}
