...
export default function HomePage() {
    ...
    async function continueAs(id: string) { const d = await action({ action: 'start', subject: 'daily' }, id); if (d) {
        setPid(id);
    ...
    <main>{error && <div className="error" role="alert">{error} <button onClick={load}>Try again</button></div>}
        ...
        {view === 'games' && <GameZone key={p.id} profile={p} busy={busy || p.preferences.paused} onStart={startGame} onContinue={()=>start('daily')} onFavorite={async(game,favorite)=>{await action({action:'favorite-game',game,favorite});}}/>}
    ...
    {play && p && !p.preferences.paused && <QuestPlayer profile={p} current={current} feedback={feedback} selected={selected} busy={busy} error={error}
      onBack={()=>{setPlay(false);window.speechSynthesis?.cancel();}} onAnswer={answer} onHint={hint} onNext={next}
