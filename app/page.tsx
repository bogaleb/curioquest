...
import {ReadingInvitation} from '@/components/reading/ReadingInvitation';
...
export default function HomePage() {
    ...
    const parentUnlocked=useCallback(()=>{setParentOpen(true);setGate(false);setViewRaw('parent');requestAnimationFrame(()=>window.scrollTo({top:0,behavior:'instant'}));},[]);
    ...
    <main>{error && <div className="error" role="alert">{error} <button onClick={load}>Try again</button></div>}
        {!p ? <div className="loading"><LoaderCircle className="spin"/> Opening your adventure…</div> : <>
            ...
            {view === 'adventure' && <ReadingInvitation onOpen={()=>setView('reading')}/>}
