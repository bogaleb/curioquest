...
function startMusic(){if(!settings.music||musicTimer)return;const notes=[261.63,329.63,392,329.63];let i=0;musicTimer=setInterval(()=>{if(typeof document!=='undefined'&&!document.hidden)tone(notes[i++%notes.length],2.5,settings.musicVolume*.045);},3000);}
